...
# 改模型尝试模型3-debertav2-xlarge-adalora-focalloss-rdrop-类别平衡采样（任务2训练加预测一起代码）
import os
import sys
import logging
import time
...
if __name__ == '__main__':
    ...
    # 加载数据
    df_train = pd.read_json('Modified_MELD_train_efr.json')
    df_val = pd.read_json('Modified_MELD_val_efr.json')
    ...
    config = DebertaConfig.from_pretrained(path, max_position_embeddings=1024)
    ...
    criterion = FocalLoss(alpha=1, gamma=5, reduction='mean', class_weights=class_weights_tensor)
...
# 加载测试数据
df_test = pd.read_json('Modified_MELD_test_efr.json')
...
config = DebertaV2Config.from_pretrained('microsoft/deberta-v2-xlarge', num_labels=2, max_position_embeddings=1024)
model = DebertScratch(config=config, sep_token_id=tokenizer.sep_token_id)
...
# 加载模型权重
checkpoint = torch.load(model_path, map_location=torch.device('cuda'))  # 确保将模型加载到正确的设备
